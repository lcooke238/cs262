...
def setup():
    ...
    while True:
        id = input("Give this server an id (0-100): ")
        try:
            clean_id = int(id)
            if (clean_id <= 100 and clean_id >= 0):
                break
        except:
            continue
    ...
    backups = []
    while True:
        backup_host = input("Backup Server IP:  (hit enter if no more)")
        backup_port = input("Backup server host: (hit enter if no more)")
        if backup_host and backup_port:
            backups.append({"host": backup_host, "port": backup_port})
        else:
            break
    return state, clean_id, backups
